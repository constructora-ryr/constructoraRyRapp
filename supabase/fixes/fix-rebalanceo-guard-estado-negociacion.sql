-- ═══════════════════════════════════════════════════════════════════════════
-- FIX: Agregar guard de estado en rebalancear_plan_financiero
--
-- Riesgo detectado: la función no verificaba el estado de la negociación,
-- lo que permitía ajustar el cierre financiero incluso en negociaciones
-- Completadas o Cerradas por Traslado.
--
-- Este fix agrega la misma verificación que tiene renunciar_negociacion:
--   estado NOT IN ('Activa', 'Suspendida') → RAISE EXCEPTION
--
-- La función completa se reproduce desde dump_dev.sql para evitar
-- divergencias. Único cambio: bloque de validación de estado insertado
-- justo después de validar negociacion_id (líneas marcadas con -- FIX).
-- ═══════════════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  -- FIX: estado de la negociación para validación
  v_negociacion_estado text;
BEGIN
  ...
  -- FIX: Validar estado de la negociación — no permitir ajustar Completadas ni Cerradas
  SELECT estado INTO v_negociacion_estado
  FROM negociaciones
  WHERE id = v_negociacion_id;

  IF v_negociacion_estado IS NULL THEN
    RAISE EXCEPTION 'Negociación no encontrada: %', v_negociacion_id;
  END IF;

  IF v_negociacion_estado NOT IN ('Activa', 'Suspendida') THEN
    RAISE EXCEPTION 'No se puede ajustar el cierre financiero de una negociación en estado "%". Solo se permiten negociaciones Activas o Suspendidas.',
      v_negociacion_estado;
  END IF;
